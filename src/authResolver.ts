...
type ProxyJump = [SSHDestination, Record<string, string>];
...
export class RemoteSSHResolver implements vscode.RemoteAuthorityResolver, vscode.Disposable {
    ...
    private sshConnection: SSHConnection | NativeSSHConnection | undefined;
    ...
    resolve(authority: string, context: vscode.RemoteAuthorityResolverContext): Thenable<vscode.ResolverResult> {
        ...
        return vscode.window.withProgress({
            ...
            try {
                ...
                const sshUser = sshDest.user || sshHostConfig['User'] || os.userInfo().username || ''; // https://github.com/openssh/openssh-portable/blob/5ec5504f1d328d5bfa64280cd617c3efec4f78f3/sshconnect.c#L1561-L1562
                const sshPort = sshHostConfig['Port'] ? parseInt(sshHostConfig['Port'], 10) : (sshDest.port || 22);
                const proxyUseFdpass = (sshHostConfig['ProxyUseFdpass'] || '').toString().toLowerCase() === 'yes';
                ...
                // Check if we have certificates - use native SSH if so
                const hasCertificate = identityKeys.some(k => k.agentSupport) || 
                    identityFiles.some(f => fs.existsSync(untildify(f) + '-cert.pub'));
                
                let proxyStream: ssh2.ClientChannel | stream.Duplex | undefined;
                const useNativeSSH = hasCertificate && !sshHostConfig['ProxyJump'];
                ...
                if (sshHostConfig['ProxyJump']) {
                    const proxyJumps: ProxyJump[] = sshHostConfig['ProxyJump'].split(',').filter(i => !!i.trim())
                        .map((jump): ProxyJump => {
                            const proxy = SSHDestination.parse(jump);
                    ...
                    if (proxyUseFdpass) {
                        this.logger.trace('ProxyUseFdpass is enabled; skipping ProxyCommand and using direct TCP connection.');
                    } else {
                    const proxyCommandValue = sshHostConfig['ProxyCommand'];
                    const proxyCommandLine = Array.isArray(proxyCommandValue)
                        ? proxyCommandValue.join(' ')
                        : String(proxyCommandValue);
                    const substituted = proxyCommandLine
                        .replace(/%h/g, sshHostName)
                        .replace(/%n/g, sshDest.hostname)
                        .replace(/%p/g, sshPort.toString())
                        .replace(/%r/g, sshUser);
                    let proxyArgs = splitShellArgs(substituted).map(stripWrappingQuotes);
                    if (!proxyArgs.length) {
                        throw new Error('ProxyCommand resolved to an empty command');
                    }
                    ...
                    child.on('error', (error) => {
                        this.logger.error(`ProxyCommand failed: ${proxyCommand}`, error);
                    });
                ...
                this.labelFormatterDisposable = vscode.workspace.registerResourceLabelFormatter({
                    ...
                    formatting: {
                        ...
                        separator: '/',
                        tildify: true,
                        workspaceSuffix: `UPLINK: ${sshDest.hostname}` + (sshDest.port && sshDest.port !== 22 ? `:${sshDest.port}` : '')
                    }
                });
    ...
    private getSSHAuthHandler(sshUser: string, sshHostName: string, identityKeys: SSHKey[], preferredAuthentications: string[]) {
        ...
        return async (methodsLeft: string[] | null, _partialSuccess: boolean | null, callback: (nextAuth: any) => void) => {
            ...
            if (methodsLeft.includes('publickey') && identityKeys.length && preferredAuthentications.includes('publickey')) {
                ...
                this.logger.info(`Trying publickey authentication: ${identityKey.filename} ${identityKey.parsedKey?.type || 'certificate'} ${identityKey.fingerprint}`);

                if (identityKey.agentSupport && this.sshAgentSock) {
                    return callback({
                        type: 'agent',
                        username: sshUser,
                        agent: new ssh2.OpenSSHAgent(this.sshAgentSock)
                    });
                }
                ...
                if (identityKey.isCertificate) {
                    // Skip certificates for now - they're causing hangs
                    return callback(null as any);
                } else {
                    // Regular key authentication
                    const keyFilePath = identityKey.filename;
                    
                    if (!await fileExists(keyFilePath)) {
                        return callback(null as any);
                    }

                    const keyBuffer = await fs.promises.readFile(keyFilePath);
            ...
            if (methodsLeft.includes('keyboard-interactive') && keyboardRetryCount > 0 && preferredAuthentications.includes('keyboard-interactive')) {
                ...
                return callback({
                    ...
                    username: sshUser,
                    prompt: async (_name: any, _instructions: any, _instructionsLang: any, prompts: any, finish: any) => {
