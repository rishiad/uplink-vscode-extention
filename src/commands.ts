...
export async function promptuplinkWindow(reuseWindow: boolean) {
  const sshConfig = await SSHConfiguration.loadFromFS();
  const configuredHosts = sshConfig.getAllConfiguredHosts();

  const host = configuredHosts.length
    ? await vscode.window.showQuickPick(configuredHosts, {
        title: "Connect to SSH Host",
        placeHolder: "Select a configured SSH host",
      })
...
export function uplinkWindow(host: string, reuseWindow: boolean) {
...
export function uplinkLocationWindow(
  host: string,
  path: string,
  reuseWindow: boolean
) {
  vscode.commands.executeCommand(
    ...
    vscode.Uri.from({
      ...
      authority: getRemoteAuthority(host),
      path,
    }),
